(function ($, ss) {
    ...
    function recognizeMobileDevices() {
        if (ss.isMobileDevice) {
        	...
        	$('#small-searchterms').attr('placeholder', 'What are you looking for?');
        	$('.con-para').text('LET US KNOW WHERE YOU LIVE!');
        	$('.forgot-password > a').text('FORGOT?');
        	$('.loginbox .m-b-lg').text('SIGN IN');
        	$('.couponbox .coupon-code input[type="submit"]').val('APPLY');
        	$('#discountcouponcode').attr('placeholder', 'ENTER PROMOCODE');
        	$('#checkout > span').text('CHECKOUT');
        	$('.new-billing-address .title > strong').text('+ ADD NEW ADDRESS');
        }
        else {
